...
{
  /* FPU settings ------------------------------------------------------------*/
  #if (__FPU_PRESENT == 1) && (__FPU_USED == 1)   //¸¡µãÔËËãµÄÅäÖÃ
    SCB->CPACR |= ((3UL << 10*2)|(3UL << 11*2));  /* set CP10 and CP11 Full Access */
  #endif
  /* Reset the RCC clock configuration to the default reset state ------------*/
  /* Set HSION bit */
  RCC->CR |= (uint32_t)0x00000001;   //»òµÈÓÚ£ºÖÃÒ»£»¾ßÌå×÷ÓÃ¿´ÖÐÎÄ²Î¿¼ÊÖ²á ¸Ã´¦ÊÇ´ò¿ªHSI  

  /* Reset CFGR register */
  RCC->CFGR = 0x00000000;  //CFGRÈ«²¿Î»µÈÓÚ0

  /* Reset HSEON, CSSON and PLLON bits */
  RCC->CR &= (uint32_t)0xFEF6FFFF;   // °´Î»ÏàÓë  ¹Ø±ÕPLL,HSE,CSS£¨°²È«¼à¿Ø£©
